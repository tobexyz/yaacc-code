...
public class JettyServletContainer implements ServletContainerAdapter {
    ...
    public static boolean isConnectionOpen(HttpServletRequest request, byte[] heartbeat) {
        ...
        Connection connection = jettyRequest.getHttpChannel().getConnection();
        Socket socket = ((SocketChannel) connection.getEndPoint().getTransport()).socket();
    ...
    synchronized public void removeConnector(String host, int port) {
        ...
        for (Connector connector : connectors) {
            if (connector instanceof ServerConnector) {
                ...
                if (serverConnector.getHost().equals(host) && serverConnector.getLocalPort() == port) {
    ...
    protected void resetServer() {
        ...
        try {
            ...
            server.stop(); // Let's wait a second for ongoing transfers to complete
