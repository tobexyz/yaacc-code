...
public class DLNAHeaders extends UpnpHeaders {
    ...
    protected void parseHeaders() {
        ...
        YaaccLogger.v(getClass().getName(), String.format("Parsing all HTTP headers for known UPnP headers: {0}", size()));
        for (Entry<String, List<String>> entry : entrySet()) {
            ...
            if (type == null) {
                YaaccLogger.v(getClass().getName(), String.format("Ignoring non-UPNP HTTP header: {0}", entry.getKey()));
                continue;
            }
            ...
            for (String value : entry.getValue()) {
                ...
                if (upnpHeader == null || upnpHeader.getValue() == null) {
                    YaaccLogger.v(getClass().getName(), String.format("Ignoring known but non-parsable header (value violates the UDA specification?) '{0}': {1}", new Object[]{type.getHttpName(), value}));
                } else {
    ...
    protected void addParsedValue(DLNAHeader.Type type, UpnpHeader value) {
        YaaccLogger.v(getClass().getName(), String.format("Adding parsed header: {0}", value));
    ...
    public void log() {
        ...
        if (parsedDLNAHeaders != null && parsedDLNAHeaders.size() > 0) {
            YaaccLogger.v(getClass().getName(), "########################## PARSED DLNA HEADERS ##########################");
            for (Map.Entry<DLNAHeader.Type, List<UpnpHeader>> entry : parsedDLNAHeaders.entrySet()) {
                YaaccLogger.v(getClass().getName(), String.format("=== TYPE: {0}", entry.getKey()));
                for (UpnpHeader upnpHeader : entry.getValue()) {
                    YaaccLogger.v(getClass().getName(), String.format("HEADER: {0}", upnpHeader));
                }
            }
        }
